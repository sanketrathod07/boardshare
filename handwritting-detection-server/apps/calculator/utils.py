...
def analyze_image(img: Image, dict_of_vars: dict):
    ...
    prompt = (
        f"You are an AI calculator designed to interpret and solve mathematical expressions and equations only. "
        f"If the image content does not directly contain mathematical calculations, expressions, or formulas, "
        f"return the exact response 'Not a mathematical expression'. "
        f"Please strictly follow this rule and do not provide any additional explanations. "
        f"Only return the result as a single-line answer, or use 'Not a mathematical expression' when applicable. "
        ...
        f"solve them strictly following the PEMDAS rule. Only return the result as a single-line, plain answer without additional explanation. "
        f"For example: "
        f"Q. 2 + 3 * 4 "
        f"(3 * 4) => 12, 2 + 12 = 14. "
        f"Q. 2 + 3 + 5 * 4 - 8 / 2 "
